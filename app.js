const menu = [
    ...
    function listItemsByCategory(category){
      ...
      if(category == 'All'){
        menu.forEach(item => {
          itemsCard.innerHTML += displayCard(item)
          }
        )
      }
      else{
      let itemsList = menu.filter(item => item.category == category)
    ...
    function searchItem(){
      ...
      const itemsCard = document.getElementById('items');
      itemsCard.innerHTML = '';
      let itemsList = menu.filter(item => item.title.toLowerCase().includes(text));
      ...
      itemsCard.innerHTML = '<h1>Item not found</h1>';
    }
  }
  listItemsByCategory("All");
